# 🖥️ Информация о виртуальной машине

Технические характеристики и детали конфигурации виртуальной машины MedHistory в Яндекс.Облаке.

---

## 📊 Основные характеристики

| Параметр | Значение |
|----------|----------|
| **ID** | `fhmsq7s4569qgl1oga4p` |
| **Имя** | `medhistory-server` |
| **Статус** | `RUNNING` |
| **Платформа** | `standard-v2` |
| **Дата создания** | 28 октября 2025, 18:46:11 MSK |

---

## 💻 Конфигурация

### Вычислительные ресурсы
- **vCPU:** 4 ядра
- **Core fraction:** 100% (гарантированная производительность)
- **RAM:** 8 GB (8589934592 bytes)

### Хранилище
- **Загрузочный диск:**
  - Тип: `network-ssd`
  - Размер: 50 GB
  - Device name: `fhm7lhk4a8ruibjj7l4e`
  - Auto-delete: `true`
  - Свободно: ~47 GB

### Операционная система
- **ОС:** Ubuntu 22.04.5 LTS (Jammy Jellyfish)
- **Kernel:** Linux
- **Архитектура:** x86_64

---

## 🌐 Сетевые настройки

### IP адреса
- **Внешний IP:** `46.21.244.23` (статический NAT)
- **Внутренний IP:** `10.128.0.19`
- **MAC адрес:** `d0:0d:1c:d1:f8:42`

### Сеть
- **Network ID:** `enpvbooknqn7tsd5bfu8`
- **Subnet ID:** `e9b47srrtuufggg2udh6`
- **Subnet name:** `medhistory-subnet-a`
- **Зона доступности:** `ru-central1-a`
- **CIDR:** `10.128.0.0/24`

### Группа безопасности
- **Security Group ID:** `enpo36phcf9ck74h6igk`
- **Имя:** `medhistory-sg`

#### Правила входящего трафика (Ingress)
| Порт | Протокол | Источник | Описание |
|------|----------|----------|----------|
| 22 | TCP | 0.0.0.0/0 | SSH |
| 80 | TCP | 0.0.0.0/0 | HTTP |
| 443 | TCP | 0.0.0.0/0 | HTTPS |

#### Правила исходящего трафика (Egress)
| Порт | Протокол | Назначение | Описание |
|------|----------|------------|----------|
| any | any | 0.0.0.0/0 | Allow all outgoing |

---

## 🔐 Настройки доступа

### SSH
- **SSH Authorization:** `OS_LOGIN` (Yandex Cloud OS Login)
- **Пользователь:** `yc-user`
- **SSH ключ:** Настроен в метаданных ВМ
- **Serial Port:** Включен

### Метаданные
```yaml
serial-port-enable: 1
ssh-keys: "yc-user:<public_key>"
```

### FQDN
- **Internal FQDN:** `fhmsq7s4569qgl1oga4p.auto.internal`

---

## 📈 Мониторинг и лимиты

### Текущее использование ресурсов
```bash
# Проверить на сервере:
ssh -l yc-user 46.21.244.23 'top -bn1 | head -20'
```

### Дисковое пространство
```bash
ssh -l yc-user 46.21.244.23 'df -h'
```

**Вывод:**
```
Filesystem      Size  Used Avail Use% Mounted on
/dev/vda1        48G  1.8G   47G   4% /
```

### Память
```bash
ssh -l yc-user 46.21.244.23 'free -h'
```

**Вывод:**
```
              total        used        free      shared  buff/cache   available
Mem:           7.8Gi       158Mi       7.0Gi       0.0Ki       564Mi       7.4Gi
Swap:             0B          0B          0B
```

---

## 🔄 Жизненный цикл ВМ

### Управление питанием
```bash
# Остановка (данные сохраняются, экономия средств)
yc compute instance stop fhmsq7s4569qgl1oga4p

# Запуск
yc compute instance start fhmsq7s4569qgl1oga4p

# Перезагрузка
yc compute instance restart fhmsq7s4569qgl1oga4p
```

### Изменение конфигурации
```bash
# Изменить количество ядер и памяти (требуется остановка ВМ)
yc compute instance update fhmsq7s4569qgl1oga4p \
  --cores 8 \
  --memory 16

# Увеличить диск (можно без остановки)
yc compute disk update fhm7lhk4a8ruibjj7l4e --size 100
```

---

## 💰 Стоимость

### Ежемесячные расходы (примерно)

| Компонент | Стоимость |
|-----------|-----------|
| **4 vCPU (100%)** | ~2,500₽/мес |
| **8 GB RAM** | Включено в стоимость vCPU |
| **50 GB SSD** | ~500₽/мес |
| **Внешний IP** | ~150₽/мес |
| **Трафик** | ~200₽/мес |
| **Итого** | **~3,350₽/мес** |

### Оптимизация расходов

1. **Остановка ВМ в нерабочее время:**
   ```bash
   # Оплата только за диск (~500₽/мес)
   yc compute instance stop medhistory-server
   ```

2. **Снижение core-fraction:**
   ```bash
   # Использовать 50% вместо 100% (экономия ~50%)
   yc compute instance update fhmsq7s4569qgl1oga4p --core-fraction 50
   ```

3. **Использование прерываемой ВМ:**
   - Скидка до 70%
   - Может быть остановлена в любой момент

---

## 📦 Резервное копирование

### Создание снимка диска
```bash
# Создать snapshot
yc compute snapshot create \
  --name medhistory-snapshot-$(date +%Y%m%d) \
  --disk-id fhm7lhk4a8ruibjj7l4e \
  --description "Backup before update"

# Список снимков
yc compute snapshot list

# Восстановление из снимка
yc compute disk create \
  --name medhistory-disk-restored \
  --source-snapshot-name medhistory-snapshot-20251028
```

### Автоматизация бэкапов
```bash
# Создать расписание снимков
yc compute snapshot-schedule create daily-backup \
  --disk-id fhm7lhk4a8ruibjj7l4e \
  --schedule-policy "start_at: '02:00', expression: '0 2 * * *'" \
  --snapshot-count 7 \
  --retention-period 168h
```

---

## 📋 Получение полной информации

### Через YC CLI
```bash
# Краткая информация
yc compute instance get medhistory-server

# Полная информация в YAML
yc compute instance get fhmsq7s4569qgl1oga4p --format yaml

# Полная информация в JSON
yc compute instance get fhmsq7s4569qgl1oga4p --format json
```

### Через API
```bash
# Получить токен
export YC_TOKEN=$(yc iam create-token)

# Запрос к API
curl -H "Authorization: Bearer $YC_TOKEN" \
  https://compute.api.cloud.yandex.net/compute/v1/instances/fhmsq7s4569qgl1oga4p
```

---

## 🔗 Полезные ссылки

- [Консоль Yandex Cloud](https://console.cloud.yandex.ru/folders/b1gudpmb327g7hf8rc5i)
- [Документация Compute Cloud](https://cloud.yandex.ru/docs/compute/)
- [Pricing Calculator](https://cloud.yandex.ru/prices)

---

## 📝 История изменений

| Дата | Изменение |
|------|-----------|
| 28.10.2025 | Создание ВМ medhistory-server |
| 28.10.2025 | Добавлен SSH ключ в метаданные |
| 28.10.2025 | Настроена группа безопасности |

---

**Последнее обновление:** 28 октября 2025

