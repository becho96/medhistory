# 🚀 Руководство по развертыванию MedHistory

Полное руководство по работе с системой развертывания MedHistory на разных средах.

## 📋 Содержание

1. [Быстрый старт](#быстрый-старт)
2. [Архитектура сред](#архитектура-сред)
3. [Первоначальная настройка](#первоначальная-настройка)
4. [Git Workflow](#git-workflow)
5. [Процесс разработки](#процесс-разработки)
6. [Тестирование на Staging](#тестирование-на-staging)
7. [Деплой на Production](#деплой-на-production)
8. [Откат изменений](#откат-изменений)
9. [Устранение проблем](#устранение-проблем)

---

## 🎯 Быстрый старт

### Для Product Manager

Вам нужны всего **два скрипта**:

```bash
# 1. Тестирование изменений локально/staging
./deploy staging

# 2. Деплой на Production
./deploy production
```

Вот и всё! 🎉

---

## 🏗 Архитектура сред

### 🔧 Local (Локальная разработка)

**Назначение:** Разработка и отладка на локальной машине  
**Расположение:** Ваш компьютер (Docker)  
**Конфигурация:** `.env.local`  
**Команда запуска:** `./deploy local`

**Особенности:**
- Hot reload для быстрой разработки
- Открытые порты для отладки
- Простые пароли (не критично)
- Минимальная безопасность

**Доступные сервисы:**
- Frontend: http://localhost:5173
- Backend API: http://localhost:8000
- API Docs: http://localhost:8000/docs
- MinIO Console: http://localhost:9001
- Grafana: http://localhost:3000 (с --monitoring)

---

### 🧪 Staging (Тестовая среда)

**Назначение:** Тестирование перед production  
**Расположение:** Локально или отдельный сервер  
**Конфигурация:** `.env.staging`  
**Команда запуска:** `./deploy staging`

**Особенности:**
- Production-like окружение
- Безопасные пароли
- Production Docker образы
- Изолированные порты (не конфликтует с local)

**Доступные сервисы:**
- Frontend: http://localhost:8080
- Backend API: http://localhost:8001
- API Docs: http://localhost:8001/docs
- MinIO Console: http://localhost:9011

---

### 🚀 Production (Боевой сервер)

**Назначение:** Реальные пользователи  
**Расположение:** Яндекс.Облако (46.21.244.23)  
**Конфигурация:** `.env.production`  
**Команда запуска:** `./deploy production`

**Особенности:**
- Максимальная безопасность
- Автоматические бэкапы
- Мониторинг и логирование
- SSL/TLS шифрование (опционально)

**Доступные сервисы:**
- Frontend: http://46.21.244.23
- API Docs: http://46.21.244.23/docs
- Grafana: http://46.21.244.23:3000

---

## 🛠 Первоначальная настройка

### Шаг 1: Настройка конфигураций

Создайте конфигурационные файлы для каждой среды:

```bash
# 1. Local конфигурация
cp environments/local.env .env.local

# 2. Staging конфигурация
cp environments/staging.env .env.staging

# 3. Production конфигурация (ВАЖНО!)
cp environments/production.env .env.production
```

### Шаг 2: Получение API ключей

**OpenRouter API Key** (обязательно):
1. Зарегистрируйтесь на https://openrouter.ai/
2. Получите API ключ: https://openrouter.ai/keys
3. Добавьте ключ в `.env.local`, `.env.staging`, `.env.production`

```bash
OPENROUTER_API_KEY=sk-or-v1-ваш-ключ-здесь
```

### Шаг 3: Генерация безопасных паролей

⚠️ **КРИТИЧНО для staging и production!**

```bash
# Генерация паролей (macOS/Linux)
openssl rand -base64 32  # PostgreSQL
openssl rand -base64 32  # MongoDB
openssl rand -base64 32  # MinIO
openssl rand -base64 48  # JWT Secret (64 символа)
```

Замените все `change_me_*` значения в `.env.staging` и `.env.production`

### Шаг 4: Проверка конфигурации

```bash
# Проверка local
./scripts/utils/validate-config.sh local

# Проверка staging
./scripts/utils/validate-config.sh staging

# Проверка production
./scripts/utils/validate-config.sh production
```

### Шаг 5: Первый запуск

```bash
# Запуск локальной среды
./deploy local

# Проверка работы
open http://localhost:5173
```

---

## 🌳 Git Workflow

### Структура веток

```
main (production)
  ↑
  └── develop (staging)
        ↑
        └── feature/название-фичи
        └── bugfix/название-багфикса
```

### Правила работы с ветками

#### 📌 `main` - Production ветка

- **Запрещено** пушить напрямую
- Только через Pull Request из `develop`
- Всегда стабильная версия
- Каждый коммит = релиз на production

#### 📌 `develop` - Staging ветка

- Интеграционная ветка
- Merge всех завершенных фич
- Тестирование перед production
- Каждый коммит = деплой на staging

#### 📌 `feature/*` - Ветки разработки

- Создаются из `develop`
- Одна ветка = одна функция
- После завершения → PR в `develop`

---

## 💻 Процесс разработки

### Сценарий: Добавление новой функции

#### 1. Создание ветки

```bash
# Обновляем develop
git checkout develop
git pull origin develop

# Создаем feature ветку
git checkout -b feature/add-user-profile

# Проверяем текущую ветку
git branch
```

#### 2. Разработка

```bash
# Запускаем локальное окружение
./deploy local

# Разрабатываем функциональность
# Редактируем код...
# Тестируем на http://localhost:5173
```

#### 3. Коммиты

```bash
# Добавляем изменения
git add .

# Коммитим с понятным сообщением
git commit -m "feat: добавлен профиль пользователя"

# Пушим в удаленный репозиторий
git push origin feature/add-user-profile
```

#### 4. Тестирование

```bash
# Останавливаем local
docker compose -f docker-compose.base.yml -f docker-compose.local.yml down

# Запускаем на staging
./deploy staging

# Тестируем на http://localhost:8080
# Проверяем все сценарии использования
```

#### 5. Pull Request в develop

```bash
# На GitHub/GitLab создаем PR:
# feature/add-user-profile → develop

# После код-ревью и одобрения
git checkout develop
git merge feature/add-user-profile
git push origin develop
```

---

## 🧪 Тестирование на Staging

### Когда использовать Staging?

- ✅ Перед каждым production деплоем
- ✅ Тестирование нескольких интегрированных фич
- ✅ QA тестирование
- ✅ Демонстрация заказчику

### Процесс тестирования

#### 1. Развертывание на Staging

```bash
# Убедитесь что вы на ветке develop
git checkout develop
git pull origin develop

# Разверните на staging
./deploy staging

# С мониторингом (опционально)
./deploy staging --monitoring
```

#### 2. Проверочный список

- [ ] Авторизация работает
- [ ] Загрузка документов работает
- [ ] AI анализ работает
- [ ] Все страницы открываются
- [ ] Нет ошибок в консоли
- [ ] API endpoints отвечают
- [ ] База данных сохраняет данные

#### 3. Проверка логов

```bash
# Просмотр логов всех сервисов
docker compose -f docker-compose.base.yml -f docker-compose.staging.yml logs -f

# Только backend
docker compose -f docker-compose.base.yml -f docker-compose.staging.yml logs -f backend

# Только frontend
docker compose -f docker-compose.base.yml -f docker-compose.staging.yml logs -f frontend
```

#### 4. Тестирование производительности

```bash
# Откройте Grafana
open http://localhost:3001

# Логин: admin / admin
# Проверьте метрики:
# - Время ответа API
# - Использование памяти
# - Количество запросов
# - Ошибки
```

---

## 🚀 Деплой на Production

### ⚠️ Чек-лист перед деплоем

- [ ] Все тесты на staging прошли успешно
- [ ] Нет критических багов
- [ ] `.env.production` настроен с безопасными паролями
- [ ] Есть бэкап текущей версии (автоматически)
- [ ] Уведомлены пользователи (если downtime)
- [ ] Команда доступна для поддержки

### Процесс деплоя

#### 1. Подготовка

```bash
# Переключаемся на develop
git checkout develop

# Создаем Pull Request: develop → main
# После одобрения, merge в main

# Обновляем main локально
git checkout main
git pull origin main

# Создаем тег версии
git tag -a v1.2.0 -m "Release v1.2.0: Добавлен профиль пользователя"
git push origin v1.2.0
```

#### 2. Валидация конфигурации

```bash
# Проверка production конфигурации
./scripts/utils/validate-config.sh production

# Если есть ошибки - исправьте их!
```

#### 3. Деплой

```bash
# Запуск деплоя на production
./deploy production

# С мониторингом
./deploy production --monitoring

# Без бэкапа (не рекомендуется!)
./deploy production --no-backup
```

#### 4. Мониторинг деплоя

Скрипт выполнит:
1. ✅ Проверку SSH подключения
2. ✅ Создание бэкапа текущей версии
3. ✅ Загрузку новых файлов
4. ✅ Остановку старых контейнеров
5. ✅ Сборку новых образов
6. ✅ Запуск новых контейнеров
7. ✅ Проверку статуса

#### 5. Проверка работы

```bash
# Откройте приложение
open http://46.21.244.23

# Проверьте API
curl http://46.21.244.23/health

# Проверьте документацию
open http://46.21.244.23/docs
```

#### 6. Мониторинг после деплоя

```bash
# Просмотр логов (первые 5 минут)
ssh yc-user@46.21.244.23 'cd ~/medhistory && docker compose -f docker-compose.base.yml -f docker-compose.prod.yml logs -f'

# Мониторинг в Grafana
open http://46.21.244.23:3000
```

---

## ⏮ Откат изменений

### Быстрый откат

Если что-то пошло не так:

```bash
# На сервере автоматически создается архив старой версии
ssh yc-user@46.21.244.23

# Найдите бэкап
ls -la ~/ | grep medhistory_old

# Откат на старую версию
cd ~
mv medhistory medhistory_broken
mv medhistory_old_YYYYMMDD_HHMMSS medhistory

# Перезапуск
cd ~/medhistory
docker compose -f docker-compose.base.yml -f docker-compose.prod.yml up -d
```

### Откат через Git

```bash
# Найдите последнюю рабочую версию
git log --oneline

# Откатитесь на нужный коммит
git checkout <commit-hash>

# Или используйте тег
git checkout v1.1.0

# Деплой старой версии
./deploy production
```

---

## 🔧 Устранение проблем

### Проблема: Не могу подключиться к production серверу

**Решение:**

```bash
# Проверка SSH ключа
ssh -v yc-user@46.21.244.23

# Проверка IP адреса
cat .env.production | grep PROD_SERVER_IP

# Проверка фаервола
ping 46.21.244.23
```

### Проблема: Docker контейнеры не запускаются

**Решение:**

```bash
# Проверка логов
docker compose logs

# Проверка ресурсов
docker system df

# Очистка неиспользуемых ресурсов
docker system prune -a

# Перезапуск Docker Desktop
```

### Проблема: Ошибки валидации конфигурации

**Решение:**

```bash
# Проверьте файл конфигурации
cat .env.staging

# Сравните с шаблоном
diff .env.staging environments/staging.env

# Регенерируйте пароли
openssl rand -base64 32
```

### Проблема: Frontend не может подключиться к Backend

**Решение:**

```bash
# Проверьте VITE_API_URL в .env файле
cat .env.local | grep VITE_API_URL

# Для local должно быть:
VITE_API_URL=http://localhost:8000

# Для staging:
VITE_API_URL=http://localhost:8001

# Для production:
VITE_API_URL=http://46.21.244.23

# Пересоберите frontend
./deploy <env> --rebuild
```

---

## 📝 Полезные команды

### Управление контейнерами

```bash
# Просмотр запущенных контейнеров
docker ps

# Просмотр всех контейнеров
docker ps -a

# Остановка всех контейнеров
docker compose -f docker-compose.base.yml -f docker-compose.local.yml down

# Удаление с томами (удалит данные!)
docker compose -f docker-compose.base.yml -f docker-compose.local.yml down -v

# Перезапуск одного сервиса
docker compose -f docker-compose.base.yml -f docker-compose.local.yml restart backend
```

### Логи и отладка

```bash
# Все логи
docker compose logs -f

# Только последние 100 строк
docker compose logs --tail=100

# Только backend
docker compose logs -f backend

# Логи на production
ssh yc-user@46.21.244.23 'cd ~/medhistory && docker compose -f docker-compose.base.yml -f docker-compose.prod.yml logs -f backend'
```

### Бэкапы

```bash
# Создание бэкапа
./scripts/utils/backup.sh

# Восстановление из бэкапа
./scripts/utils/restore.sh <backup-file>

# Просмотр бэкапов
ls -lh ~/backups/
```

### Git команды

```bash
# Текущий статус
git status

# История коммитов
git log --oneline --graph

# Создание ветки
git checkout -b feature/название

# Обновление веток
git fetch --all
git pull origin develop

# Удаление локальной ветки
git branch -d feature/название

# Удаление удаленной ветки
git push origin --delete feature/название
```

---

## 🎓 Best Practices

### ✅ DO (Делайте так)

- ✅ Всегда тестируйте на staging перед production
- ✅ Используйте безопасные пароли для staging и production
- ✅ Создавайте осмысленные commit message
- ✅ Делайте маленькие, атомарные коммиты
- ✅ Проверяйте логи после деплоя
- ✅ Следите за метриками в Grafana
- ✅ Делайте бэкапы перед production деплоем
- ✅ Используйте теги для релизов

### ❌ DON'T (Не делайте так)

- ❌ Не пушьте напрямую в main
- ❌ Не используйте дефолтные пароли на production
- ❌ Не деплойте без тестирования
- ❌ Не игнорируйте ошибки валидации
- ❌ Не деплойте в пятницу вечером 😅
- ❌ Не удаляйте бэкапы без необходимости
- ❌ Не комитьте .env файлы

---

## 📞 Получение помощи

### Документация

- [API Documentation](./API_DOCUMENTATION.md)
- [Change Log](./change_log/)
- [Monitoring Guide](./MONITORING_DEPLOYMENT_PLAN.md)

### Проверка состояния

```bash
# Статус всех сервисов
./scripts/utils/status.sh

# Диагностика production
./scripts/prod/diagnose.sh
```

### Связь

- **Техническая поддержка:** Создайте issue в репозитории
- **Срочные проблемы:** Свяжитесь с DevOps командой

---

## 🎉 Готово!

Теперь у вас есть полное понимание процесса развертывания MedHistory.

**Ключевые команды для запоминания:**

```bash
./deploy local       # Локальная разработка
./deploy staging     # Тестирование
./deploy production  # Production деплой
```

Удачи! 🚀

