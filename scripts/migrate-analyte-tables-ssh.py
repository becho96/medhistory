#!/usr/bin/env python3
"""
Скрипт для миграции таблиц справочника анализов из локальной БД в продакшн через SSH туннель.
...
Скрипт:
1. Создает SSH туннель до продакшн сервера
2. Проверяет наличие данных в локальной БД
3. Проверяет, что продакшн таблицы пустые
4. Копирует данные с сохранением UUID и связей
"""
...
class SSHTunnel:
    ...
    def __init__(self, ssh_host: str, ssh_user: str, remote_host: str, remote_port: int, local_port: int):
    ...
    def start(self):
        ...
        print_info(f"Создание SSH туннеля: localhost:{self.local_port} -> {self.ssh_host}:{self.remote_port}")
        ...
        print_info("Ожидание установки туннеля...")
        time.sleep(3)
        
        if self.process.poll() is not None:
            _, stderr = self.process.communicate()
            raise Exception(f"Не удалось создать SSH туннель: {stderr.decode()}")
        
        print_success("SSH туннель установлен")
    
    def stop(self):
        """Останавливает SSH туннель"""
        if self.process:
            print_info("Закрытие SSH туннеля...")
            self.process.send_signal(signal.SIGTERM)
            self.process.wait(timeout=5)
            print_success("SSH туннель закрыт")
...
async def main():
    """Основная функция"""
    print_header("🚀 Миграция справочника анализов: LOCAL → PRODUCTION (через SSH)")
    ...
    # Получаем параметры локальной БД
    ...
    # Получаем параметры продакшн БД
    prod_postgres_user = os.getenv('POSTGRES_USER')
    prod_postgres_password = os.getenv('POSTGRES_PASSWORD')
    prod_postgres_db = os.getenv('POSTGRES_DB', 'medhistory')
    
    if not prod_postgres_user or not prod_postgres_password:
    ...
    ssh_user = 'yc-user'
    local_tunnel_port = 15432  # Локальный порт для туннеля
    
    # URL для подключения через туннель
    prod_db_url = f"postgresql://{prod_postgres_user}:{prod_postgres_password}@localhost:{local_tunnel_port}/{prod_postgres_db}"
    
    print_info(f"Локальная БД: {local_db_url.split('@')[1] if '@' in local_db_url else 'не определена'}")
    print_info(f"Продакшн БД: через SSH туннель на {ssh_host}")
    
    # Создаем SSH туннель
    tunnel = SSHTunnel(
        ssh_host=ssh_host,
        ssh_user=ssh_user,
        remote_host='localhost',  # PostgreSQL внутри Docker на сервере
        remote_port=5432,
    ...
    try:
        ...
        # Подключаемся к БД
        print()
        print_info("Подключение к базам данных...")
        ...
        try:
            ...
            if not await check_production_empty(prod_conn):
                ...
                print_warning("\nЕсли вы хотите перезаписать данные, сначала очистите таблицы на сервере:")
                print_warning("  ssh yc-user@158.160.99.232 'docker exec medhistory-postgres-1 psql -U medhistory_user -d medhistory'")
